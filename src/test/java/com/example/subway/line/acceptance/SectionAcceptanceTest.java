...
public class SectionAcceptanceTest extends AcceptanceTest {
    ...
    void addLineSection() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선에_지하철_구간_생성_요청(신분당선, createSectionCreateParams(성수역, 정자역));

        // then
        assertThat(response.statusCode()).isEqualTo(HttpStatus.NO_CONTENT.value());

        // then
        ExtractableResponse<Response> responseLine = 지하철_노선_조회_요청(신분당선);
        assertThat(responseLine.statusCode()).isEqualTo(HttpStatus.OK.value());
        assertThat(responseLine.jsonPath().getList("stations.id", Long.class)).containsExactly(강남역, 성수역, 정자역);
    }
...
//
//        // when
//        SectionStep.지하철_노선에_지하철_구간_제거_요청(신분당선, 정자역);
//
//        // then
